...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
  ...
  config.vm.provision "shell", path: "~/script.sh"
    ...
    config.vm.define "server" do |server|
      ...
      server.vm.synced_folder "." , "/vagrant/consul_template"
  ...
  (1..NUMBER_OF_WORKERS).each do |instance_number|
    ...
    config.vm.define "worker-#{instance_number}" do |worker|
      ...
      worker.vm.hostname = "worker-#{instance_number}"
  	  worker.vm.synced_folder "." , "/vagrant/consul_template"
      ...
      worker.vm.provision "shell", inline: "printf 'export SERVER_IP=#{SERVER_IP}\n'  >> ~/.profile"
      worker.vm.provision "shell", inline: "printf 'export NAME=$(hostname)'  >> ~/.profile"
